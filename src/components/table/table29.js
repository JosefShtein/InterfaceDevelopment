...
class RectangleGrid extends React.Component {
    ...
    renderRectangles() {
        ...
        for (let i = 0; i < 12; i++) {
            ...
            let two = this.elementsArray[3];
            let three = this.elementsArray[4];
            let four = this.elementsArray[7];
            let five = this.elementsArray[5];
            ...
            rectangles.push(
                <div key={i} style={{ display: 'inline-block', margin: '5px', position: 'relative',  left:'0px'}}>
                    ...
                    <h1 className="image-text">{datalist5["id_slab"]}</h1></div>}
                </div>
            );
        }
