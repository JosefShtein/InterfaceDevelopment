...
import data from './jsons/sheets14.json';
import './table2.css';
...
class RectangleGrid extends React.Component {
    ...
    renderRectangles() {
        ...
        for (let i = 0; i < 12; i++) {
            ...
            let two = this.elementsArray[2];
            let three = this.elementsArray[3];
            let four = this.elementsArray[4];
            let five = this.elementsArray[7];
            ...
            rectangles.push(
                <div key={i} style={{ display: 'inline-block', margin: '5px', position: 'relative',  left:'0px'}}>
                    ...
                    {one == i && <div style={{position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}><img src={require('./imOrPov3.png')}/>
                    <h1 className="image-text">{datalist0["id_slab"]}</h1></div>}
                    {two == i && <div style={{position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}><img src={require('./imOrPov4.png')}/>
                    <h1 className="image-text">{datalist1["id_slab"]}</h1></div>}
                    {three == i && <div style={{position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}><img src={require('./imOrPov3.png')}/>
                    <h1 className="image-text">{datalist2["id_slab"]}</h1></div>}
                    {four == i && <div style={{position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}><img src={require('./imOrPov2.png')}/>
                    <h1 className="image-text">{datalist3["id_slab"]}</h1></div>}
                    {five == i && <div style={{position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}><img src={require('./imOrPov2.png')}/>
                    <h1 className="image-text">{datalist4["id_slab"]}</h1></div>}
                </div>
            );
        }
